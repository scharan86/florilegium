...
$$ \alpha \le true \ minimax \ value \le \beta $$
Here, 
$\alpha =$ Lower bound on the final value achieved by max player along the current path (best score the max player can guarantee so far) 
$\beta =$ Upper bound on the final value achieved by the min player along the current path (best score minimizing player can guarantee so far)
## Algorithm 
```
